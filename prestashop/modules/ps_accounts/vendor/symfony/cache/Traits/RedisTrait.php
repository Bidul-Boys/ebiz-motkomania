...
use PrestaShop\Module\PsAccounts\Vendor\Predis\Connection\Aggregate\ClusterInterface;
use PrestaShop\Module\PsAccounts\Vendor\Predis\Connection\Aggregate\RedisCluster;
use PrestaShop\Module\PsAccounts\Vendor\Predis\Connection\Factory;
use PrestaShop\Module\PsAccounts\Vendor\Predis\Response\Status;
...
trait RedisTrait
{
    ...
    private function init($redisClient, $namespace = '', $defaultLifetime = 0)
    {
        ...
        if (!$redisClient instanceof \Redis && !$redisClient instanceof \RedisArray && !$redisClient instanceof \RedisCluster && !$redisClient instanceof \PrestaShop\Module\PsAccounts\Vendor\Predis\Client && !$redisClient instanceof RedisProxy) {
    ...
    public static function createConnection($dsn, array $options = [])
    {
        ...
        if (null === $params['class'] && !\extension_loaded('redis') && !\class_exists(\PrestaShop\Module\PsAccounts\Vendor\Predis\Client::class)) {
            throw new CacheException(\sprintf('Cannot find the "redis" extension, and "predis/predis" is not installed: "%s".', $dsn));
        }
        $class = null === $params['class'] ? \extension_loaded('redis') ? \Redis::class : \PrestaShop\Module\PsAccounts\Vendor\Predis\Client::class : $params['class'];
        ...
        } elseif (\is_a($class, \PrestaShop\Module\PsAccounts\Vendor\Predis\Client::class, \true)) {
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \PrestaShop\Module\PsAccounts\Vendor\Predis\Client && $this->redis->getConnection() instanceof ClusterInterface) {
    ...
    protected function doClear($namespace)
    {
        ...
        if ($this->redis instanceof \PrestaShop\Module\PsAccounts\Vendor\Predis\Client) {
            $evalArgs = [0, $namespace];
            $connection = $this->redis->getConnection();
            if ($connection instanceof ClusterInterface && $connection instanceof \Traversable) {
                $hosts = [];
                foreach ($connection as $c) {
                    $hosts[] = new \PrestaShop\Module\PsAccounts\Vendor\Predis\Client($c);
                }
            }
        ...
        foreach ($hosts as $host) {
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \PrestaShop\Module\PsAccounts\Vendor\Predis\Client ? $host->scan($cursor, 'MATCH', $namespace . '*', 'COUNT', 1000) : $host->scan($cursor, $namespace . '*', 1000);
    ...
    protected function doSave(array $values, $lifetime)
    {
        ...
        foreach ($results as $id => $result) {
            if (\true !== $result && (!$result instanceof Status || $result !== Status::get('OK'))) {
                $failed[] = $id;
            }
        }
        return $failed;
    ...
    {
        $ids = [];
        if ($this->redis instanceof \RedisCluster || $this->redis instanceof \PrestaShop\Module\PsAccounts\Vendor\Predis\Client && $this->redis->getConnection() instanceof RedisCluster) {
        ...
        } elseif ($this->redis instanceof \PrestaShop\Module\PsAccounts\Vendor\Predis\Client) {
