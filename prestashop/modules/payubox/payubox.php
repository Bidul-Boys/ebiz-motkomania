...
class PAYUBOX extends Module
{
	public function __construct()
	{
		$this->name = 'payubox';
		$this->description = 'pay_u_box';
		$this->tab = 'front_office_features';
		$this->version = '1.1.4';
		$this->author = 'Miguel Costa';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Payubox_MONOLANGUAGE';
		$this->text_editor_content = 'Payubox_TEXTEDITOR';
		$this->content_wrapper = 'Payubox_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Payubox_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Payubox_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('pay_u_box');
		$this->description = $this->l('pay_u_box');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !PAYUBOXModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = PAYUBOXModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = PAYUBOXModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					PAYUBOXModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					PAYUBOXModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																PAYUBOXModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class PAYUBOXModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( PAYUBOXModel::createFilesTable()
				&& PAYUBOXModel::createContentTable());
	}
