...
class PRODUCERSEARCH extends Module
{
	public function __construct()
	{
		$this->name = 'producersearch';
		$this->description = 'producer_search';
		$this->tab = 'front_office_features';
		$this->version = '1.1.4';
		$this->author = 'Miguel Costa';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Producersearch_MONOLANGUAGE';
		$this->text_editor_content = 'Producersearch_TEXTEDITOR';
		$this->content_wrapper = 'Producersearch_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Producersearch_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Producersearch_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('producer_search');
		$this->description = $this->l('producer_search');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !PRODUCERSEARCHModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !PRODUCERSEARCHModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = PRODUCERSEARCHModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = PRODUCERSEARCHModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					PRODUCERSEARCHModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					PRODUCERSEARCHModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																PRODUCERSEARCHModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class PRODUCERSEARCHModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'producersearch',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( PRODUCERSEARCHModel::createFilesTable()
				&& PRODUCERSEARCHModel::createContentTable());
	}
