...
class PRODUCTSEARCH extends Module
{
	public function __construct()
	{
		...
		$this->description = 'product_search';
		$this->tab = 'front_office_features';
		$this->version = '1.1.4';
		$this->author = 'Miguel Costa';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Productsearch_MONOLANGUAGE';
		$this->text_editor_content = 'Productsearch_TEXTEDITOR';
		$this->content_wrapper = 'Productsearch_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Productsearch_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Productsearch_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('product_search');
		$this->description = $this->l('product_search');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !PRODUCTSEARCHModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = PRODUCTSEARCHModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = PRODUCTSEARCHModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					PRODUCTSEARCHModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					PRODUCTSEARCHModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																PRODUCTSEARCHModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class PRODUCTSEARCHModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( PRODUCTSEARCHModel::createFilesTable()
				&& PRODUCTSEARCHModel::createContentTable());
	}
