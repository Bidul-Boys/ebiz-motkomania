...
class FBBOX extends Module
{
	public function __construct()
	{
		$this->name = 'fbbox';
		$this->description = 'fb_box';
		$this->tab = 'front_office_features';
		$this->version = '1.1.4';
		$this->author = 'Miguel Costa';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Fbbox_MONOLANGUAGE';
		$this->text_editor_content = 'Fbbox_TEXTEDITOR';
		$this->content_wrapper = 'Fbbox_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Fbbox_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Fbbox_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('fb_box');
		$this->description = $this->l('fb_box');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !FBBOXModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = FBBOXModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = FBBOXModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					FBBOXModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																FBBOXModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class FBBOXModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( FBBOXModel::createFilesTable()
				&& FBBOXModel::createContentTable());
	}
