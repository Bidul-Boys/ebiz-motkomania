...
class INPOSTBOX extends Module
{
	public function __construct()
	{
		$this->name = 'inpostbox';
		$this->description = 'inpost_box';
		$this->tab = 'front_office_features';
		$this->version = '1.1.4';
		$this->author = 'Miguel Costa';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Inpostbox_MONOLANGUAGE';
		$this->text_editor_content = 'Inpostbox_TEXTEDITOR';
		$this->content_wrapper = 'Inpostbox_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Inpostbox_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Inpostbox_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('inpost_box');
		$this->description = $this->l('inpost_box');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !INPOSTBOXModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = INPOSTBOXModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = INPOSTBOXModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					INPOSTBOXModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					INPOSTBOXModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																INPOSTBOXModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class INPOSTBOXModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( INPOSTBOXModel::createFilesTable()
				&& INPOSTBOXModel::createContentTable());
	}
